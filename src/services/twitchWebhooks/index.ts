...
export default class TwitchWebhooksServer {
    ...
    private toggleTwitchWebhook = async (webhook, mode) => {
        ...
        await axios
            ...
            .catch(reason =>
                BeastieLogger.warn(
                    `toggleTwitchWebhook, ${topicUrl} failed: ${reason}`
                )
            );
    };
    ...
    public confirmWebhook = async ctx => {
        BeastieLogger.info(
            `CONFIRM Webhook - hub.mode: [${
                ctx.query[`hub.mode`]
            }], hub.topic: [${ctx.query[`hub.topic`]}]`
        );
        const hubTopic = ctx.query[`hub.topic`]
            .substr(28)
            .replace(/%2F/gi, "/");
    ...
    public getActiveWebhooks = async () => {
        const subscriptions = await axios
            .get(
                "https://api.twitch.tv/helix/webhooks/subscriptions?first=100",
                {
                    headers: {
                        "Client-ID": `${config.CLIENT_ID}`,
                        Authorization: `Bearer ${config.BROADCASTER_OAUTH}`
